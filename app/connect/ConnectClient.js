...
export default function ConnectClient({ data, settings }) {
    ...
    return (
        <main ref={mainRef} style={{ backgroundColor: 'var(--color-cream)' }}>
            <Nav />
            <PageHero
                title={data?.heroHeadline || "CONNECT\nWITH US"}
                image={data?.heroImage || "/images/img_contact-header.jpg"}
            />
                    ...
                    <div className="editorial-layout">
                        <div className="reveal-up">
                            <h2 className="serif reveal-wipe" style={{ fontSize: '2.4rem', lineHeight: '1.3', marginBottom: '3rem', textTransform: 'none', fontWeight: '400', color: 'var(--color-gold)', letterSpacing: 'normal', maxWidth: '100%' }}>
                                <span className="reveal-text-wipe connect-title-wipe" style={{ display: 'block', whiteSpace: 'pre-line' }}>
                                    {data?.introHeadline || "We’re here to guide\nyour next step."}
                                </span>
                            </h2>
                        ...
                        <div className="reveal-up" style={{ paddingTop: '1rem' }}>
                            {typeof data?.introCopy === 'string' || !data?.introCopy ? (
                                <p className="body-lg" style={{ color: 'var(--color-teal)' }}>
                                    {data?.introCopy || "Whether you need new coverage or want a clear read on your current plan, we take the time to understand your goals and design the right solution. Life insurance is too personal for a contact form. We prefer a conversation."}
                                </p>
                            ) : (
                                <PortableText 
