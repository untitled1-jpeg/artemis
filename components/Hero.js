...
export default function Hero({ data }) {
    ...
    useLayoutEffect(() => {
        const ctx = gsap.context(() => {
            ...
            // 2. Dynamic Logo Cross-Fade on Scroll
            gsap.to(logoRef.current, {
                scrollTrigger: {
                    ...
                    onUpdate: (self) => {
                        ...
                        // Subtle fade out for hero logo
                        if (logo) {
                            logo.style.opacity = Math.max(0, 1 - self.progress * 2.5);
                            logo.style.transform = `translateY(${-self.progress * 60}px)`;
                        }

                        // Trigger the nav logo fade-in via CSS class
                        if (nav) {
        ...
        <header className="hero" ref={heroRef}>
            <div className="container">
                <div
                    ...
                    id="hero-logo-master"
                    style={{ width: '200px', zIndex: 2000, opacity: 0, marginBottom: '2rem' }}
                >
                ...
                <h1 ref={headlineRef} style={{ opacity: 0, transform: 'translateY(40px)', whiteSpace: 'pre-line', textAlign: 'left', marginLeft: 0 }}>{content.headline}</h1>
                <p ref={paragraphRef} style={{ opacity: 0, transform: 'translateY(20px)', textAlign: 'left', marginLeft: 0 }}>{content.subheadline}</p>
            </div>
