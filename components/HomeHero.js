...
export default function HomeHero({ data }) {
    ...
    useLayoutEffect(() => {
        const ctx = gsap.context(() => {
            const tl = gsap.timeline({ defaults: { ease: 'power3.out', duration: 1.5 }, delay: 0.2 });

            // Entrance Sequence (matching PageHero style but for home elements)
            tl.to(logoRef.current, { opacity: 1, duration: 1 })
                .to(titleRef.current, { opacity: 1, y: 0, clipPath: 'inset(0% 0 0 0)' }, "-=0.5")
                .to(textRef.current, { opacity: 0.9, y: 0 }, "-=1");
    ...
    return (
        <header className="page-hero home-hero-v2" ref={heroRef} style={{
            ...
            <div className="container" style={{ position: 'relative', zIndex: 3, width: '100%' }}>
                ...
                <div ref={logoRef} className="hero-logo-container" style={{ width: '400px', marginBottom: '2.5rem', opacity: 0 }}>
